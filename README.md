...
- [`i18n/`](i18n/) — the **subjects** of the eulogies: the saint, blessed or celebration each eulogy is directed to, in nominative display form per language (`la.json` complete — "Sancta Maria Dei Genetrix", "Sanctus Basilius" — including the deprecated IDs; `it.json` and `en.json` partial, extracted mechanically from the 2004 edition texts and awaiting translator completion). Subject and slug are tightly coupled and edition-independent: the eulogy text may change between editions while subject and canonical ID remain the same.
